...
faceValues = [['A', 14],['K',13],['Q',12],['T', 10],['J', '01']]
...
def getFaceCard (cardNum):
    if cardNum > 1 and cardNum < 10:
        return str(cardNum)
    elif cardNum == 1:
        return 'J'
    else:
        faceCardValue = list(filter(lambda x: x[1] == cardNum, faceValues))[0][0]
...
def appendHandType(input):
    def makeCountObj(hand):
        ...
        for card in arr:
            if results[card] == 0 and card != 'J':
                del results[card]
        counts = list(results.values())
        counts.sort(reverse=True)
        mostCounted = counts[0]
        keys = [k for k, v in results.items() if v == mostCounted]
        keyValues = list(map(getCardValue, keys))
        keyValues = (list(map(lambda x: int(x),keyValues)))
        keyValues.sort(reverse=True)
        mostCountedCard = (getFaceCard(keyValues[0]))
        for i in range(results['J']):
            results[mostCountedCard] = results[mostCountedCard] + 1
        if results == {"J": 10} :
            return {"J": 5}
        else:
            del results['J']
...
def sortCollections(groupedCards):
    def sortCollection(collection):
        sortedCollection = sorted(collection, key=lambda x: int(x['numberValue']), reverse=True)
