...
def predictPrevValues(sequences):
    def predictPrevValue(sequence):
        reversedList = sequence[::-1]
        result = 0
        for index, listItem in enumerate(reversedList):
            if index > 0:
                firstListItem = listItem[0]
                result = firstListItem - result
        return result
    prevValues = []
    for sequence in sequences:
        prevValues.append(predictPrevValue(sequence))
    return prevValues

inputArr = inputToArray(chosenPath)
sequences = createSequences(inputArr)
#nextValues = predictNextValues(sequences)
prevValues = predictPrevValues(sequences)
