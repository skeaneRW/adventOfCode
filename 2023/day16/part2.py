        ...
        match thisValue:
            ...
            case "-":
                if dir in ['left', 'right']:
                    ...
                    if (x, y, 'left') not in visitedBeams:
                        additionalBeams.append((x, y, 'left'))
                    if (x, y, 'right') not in visitedBeams:
...
def getScore(startX, startY, startDir):
    gridAreasCovered = []
    pendingBeams = []
    visitedBeams = []
    beamPath, additionalBeams = createBeam(startX, startY, startDir, visitedBeams)
    ...
    return(len(energizedGridAreas))

entryPoints = []

for x in range(len(grid[0])):
    entryPoints.append((0, x, 'down'))
    entryPoints.append((len(grid)-1, x, 'up'))
for y in range(len(grid)):
    entryPoints.append((y, 0, 'right'))
    entryPoints.append((y, len(grid[0])-1, 'right'))
highScore = 0
for i, entryPoint in enumerate(entryPoints):
    #print(f'{i} of {len(entryPoints)}')
    x, y, dir = entryPoint
    score = getScore(x, y, dir)
    if score > highScore:
        highScore = score
print(highScore)